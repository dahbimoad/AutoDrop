...
.DESCRIPTION
    ...
    Version is read from AutoDrop.csproj by default (single source of truth).
.PARAMETER Version
    Version number (default: reads from csproj)
.PARAMETER Architecture
    Target architecture: x64, x86 (default: x64)
.EXAMPLE
    .\build-inno.ps1
    .\build-inno.ps1 -Version "1.2.0" -Architecture "x64"
#>
...
$CsprojPath = Join-Path $ProjectRoot "AutoDrop\AutoDrop.csproj"

# Read version from csproj if not provided (single source of truth)
if (-not $Version) {
...
Write-Host "  Version: $Version (from csproj)" -ForegroundColor Cyan
Write-Host "  Architecture: $Architecture" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# Step 1: Check Inno Setup
Write-Host "[1/3] Checking Inno Setup..." -ForegroundColor Yellow
